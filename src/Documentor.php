...
class Documentor {
    ...
    public function __construct(Filesystem $fs) {
        $this->fs = $fs;
        // $this->withSchema(config('documentor.versions.open_api'));
    }
    ...
    public function createDocsDirectory() {
        if ( ! $this->fs->isDirectory(config('documentor.save.path'))) {
            $this->fs->makeDirectory(config('documentor.save.path'), 0755, true);
        }
    }
    ...
    public function save() {
        if ($this->isValid) {
            ...
            $this->fs->put(config('documentor.save.path').'/'.config('documentor.save.filename'), $docs);
